...
def opendb(dbname, classname):
    ...
    sql = "create table if not exists s2020" + str(classname) + """ (LATESTEXAM TEXT NOT NULL,
...
def read_db_exam_banciduanci(dbname, classname, stuname, graphics, stuid, dbstr):
    ...
    if graphics == '语文':
        ...
        sql = "select wuzhengbanci, wuzhengduanci from " + dbstr + str(stuid) + " where name = '" + stuname + "' ORDER BY rowid DESC LIMIT 2"
        sql2 = "select wuzhengduanci from " + dbstr + str(classname) + " where name = '一本线' ORDER BY rowid DESC LIMIT 2"
    elif graphics == '历史':
        sql = "select huashibanci, huashiduanci from " + dbstr + str(stuid) + " where name = '" + stuname + "' ORDER BY rowid DESC LIMIT 2"
        sql2 = "select huashiduanci from " + dbstr + str(classname) + " where name = '一本线' ORDER BY rowid DESC LIMIT 2"
    elif graphics == '地理':
        sql = "select shengdibanci, shengdiduanci from " + dbstr + str(stuid) + " where name = '" + stuname + "' ORDER BY rowid DESC LIMIT 2"
        sql2 = "select shengdiduanci from " + dbstr + str(classname) + " where name = '一本线' ORDER BY rowid DESC LIMIT 2"
    elif graphics == '物理':
        sql = "select wuzhengbanci, wuzhengduanci from " + dbstr + str(stuid) + " where name = '" + stuname + "' ORDER BY rowid DESC LIMIT 2"
        sql2 = "select wuzhengduanci from " + dbstr + str(classname) + " where name = '一本线' ORDER BY rowid DESC LIMIT 2"
    elif graphics == '化学':
        ...
        sql = "select liwenbanci, liwenduanci from " + dbstr + str(stuid) + " where name = '" + stuname + "' ORDER BY rowid DESC LIMIT 2"
        sql2 = "select liwenduanci from " + dbstr + str(classname) + " where name = '一本线' ORDER BY rowid DESC LIMIT 2"
    elif graphics == '文综':
        sql = "select liwenbanci, liwenduanci from " + dbstr + str(stuid) + " where name = '" + stuname + "' ORDER BY rowid DESC LIMIT 2"
        sql2 = "select liwenduanci from " + dbstr + str(classname) + " where name = '一本线' ORDER BY rowid DESC LIMIT 2"
...
def adddb(dbname):
    ...
    for tmp in data:
        ...
        xdata = []
        for x in pname:
            xdata,ydata = read_db_exam_banciduanci("20ji.db", classname, stuname, x, stuid, "s2020")
            #print("xdata",xdata)
            #print("ydata",ydata)
        ...
        if not breakflag:
            ...
            sql = "DELETE FROM s2020" + str(stuid)
            print(sql)
            hel[1].execute(sql)
            hel[1].commit()
            
            
            sql = "insert into s2020" + str(stuid) + """(LATESTEXAM,
