...
def opendb(dbname, classname):
    ...
    sql = "create table if not exists s2020" + str(classname) + """ (EXAM TEXT NOT NULL,
...
def adddb(dbname):
    welcome = """--------------------欢迎使用添加数据功能-----------------------"""
    print(welcome)
    data = readxlsx('./2020级/analysis/20230410/1gaosanbanjifenxi.xlsx', 'Sheet1')
    classname = ""
    for tmp in data:
        if tmp == data[0]:
            continue
        ...
        sql = "insert into s2020" + str(classname) + """(EXAM, SUM,
