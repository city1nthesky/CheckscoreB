...
def opendb(dbname, classname):
    ...
    sql = "create table if not exists s2020" + str(classname) + """ (NAME TEXT NOT NULL, EXAM TEXT,
           ...
           WUZHENG TEXT, WUZHENGBANCI TEXT, WUZHENGDUANCI TEXT,
           HUASHI TEXT, HUASHIBANCI TEXT, HUASHIDUANCI TEXT,
           SHENGDI TEXT, SHENGDIBANCI TEXT, SHENGDIDUANCI TEXT,
           LIWEN TEXT, LIWENBANCI TEXT, LIWENDUANCI TEXT)"""
...
def adddb(dbname):
    welcome = """--------------------欢迎使用添加数据功能-----------------------"""
    print(welcome)
    data = readxlsx('./2020级/20230424/3gaosanermoyibenxianzuigaofen.xlsx', 'Sheet1')
    classname = ""
    for tmp in data:
        if tmp == data[0]:
            continue
        ...
        sql = "insert into s2020" + str(classname) + """(NAME, EXAM, SUM, SUMBANCI, SUMDUANCI,
               YUWEN, YUWENBANCI, YUWENDUANCI, SHUXUE, SHUXUEBANCI, SHUXUEDUANCI, YINGYU, YINGYUBANCI, YINGYUDUANCI,
               WUZHENG, WUZHENGBANCI, WUZHENGDUANCI, HUASHI, HUASHIBANCI, HUASHIDUANCI, SHENGDI, SHENGDIBANCI, SHENGDIDUANCI,
               LIWEN, LIWENBANCI, LIWENDUANCI)values(""" + "\""+ str(tmp[1]) + "\", \"" + str(tmp[2])+ "\"," + str(tmp[3]) +\
