...
def opendb(dbname, classname):
    ...
    sql = "create table if not exists s2022" + str(classname) + """ (GRADE TEXT NOT NULL, EXAM TEXT NOT NULL,
...
def adddb(dbname):
    welcome = """--------------------欢迎使用添加数据功能-----------------------"""
    print(welcome)
    data = readxlsx('./2022级/analysis/20230411/1gaoyijibuxuekefenxi.xlsx', 'Sheet1')
    classname = ""
    for tmp in data:
        if tmp == data[0]:
            continue
        ...
        sql = "insert into s2022" + str(classname) + """(GRADE, EXAM, SUM,
               YUWEN, SHUXUE, YINGYU)values(""" + "\""+ str(tmp[1]) + "\", \"" + str(tmp[2])+ "\"," + str(tmp[3]) +"," + str(tmp[4])\
                       + "," + str(tmp[5]) + "," + str(tmp[6]) +")"
        print(sql)
        hel[1].execute(sql)
        hel[1].commit()
