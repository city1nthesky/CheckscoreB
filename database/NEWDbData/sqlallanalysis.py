...
def opendb(dbname, classname):
    ...
    sql = "create table if not exists s2021" + str(classname) + """ (NAME TEXT NOT NULL,
           ALLEXAM INTEGER, SUM INTEGER, SUNRATE REAL, YUWEN INTEGER, SHUXUE INTEGER, YINGYU INTEGER,
...
def adddb(dbname):
    welcome = """--------------------欢迎使用添加数据功能-----------------------"""
    print(welcome)
    data = readxlsx('./2021级/1gaoerbanjixingmingxuehaoquan2buchong.xlsx', 'Sheet1')
    classname = ""
    for tmp in data:
        if tmp == data[0]:
            continue
        ...
        sql = "insert into s2021" + str(stuid) + """(NAME, ALLEXAM, SUM,
