...
def opendb(dbname, classname):
    ...
    sql = "create table if not exists s2022" + classname + " (STUID TEXT NOT NULL, NAME TEXT)"
...
def adddb(dbname):
    welcome = """--------------------欢迎使用添加数据功能-----------------------"""
    print(welcome)
    data = readxlsx('./newgaoyi/班级学号名字.xlsx', 'Sheet1')
    classname = "2101"
    for tmp in data:
        if tmp == data[0]:
            continue
        ...
        sql = "insert into s2022" + classname + """(STUID, NAME)values(""" +  str(tmp[1]) + ", \"" + str(tmp[2])+ "\")" 
        print(sql)
        hel[1].execute(sql)
        hel[1].commit()
        hel[1].close()
