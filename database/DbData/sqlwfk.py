...
def opendb(dbname, classname):
    ...
    sql = "create table if not exists w2022" + classname + """ (NAME TEXT NOT NULL, EXAM TEXT,
...
def adddb(dbname):
    welcome = """--------------------欢迎使用添加数据功能-----------------------"""
    print(welcome)
    data = readxlsx('test.xlsx', 'Sheet1')
    classname = ""
    for tmp in data:
        if tmp == data[0]:
            continue
        ...
        sql = "insert into w2022" + classname + """(NAME, EXAM, SUM, SUMBANCI, SUMDUANCI,
